...
- `vercel.json` builds the web app (`pnpm build:web`, output `apps/web/dist`) and rewrites non-API routes to `/index.html` (SPA fallback). The Vercel build does not run the API server; the API runs as a serverless function instead.
- `api/[[route]].ts` is the Vercel function that serves every `/api/*` request with the same Hono application as `pnpm start` (`createApiApp` + `runNordhausBootstrap`, bootstrapped once per warm instance). Local single-port mode (`pnpm start`) is unchanged.
